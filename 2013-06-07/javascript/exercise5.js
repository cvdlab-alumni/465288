...
function createRoads(rows,cols){
  var roads = null;
  var xdistance = 3;
  var zdistance = 3;
  for(c=0;c<cols-1;c++){
    var road = COLOR(neroChiaro)(CUBOID([1,0.1,rows*4]));
    road = T([X])([xdistance])(road);
    xdistance = xdistance + 4;
    if(roads===null) roads = road;
    else roads = STRUCT([roads,road]);
  }
  for(r=0;r<rows-1;r++){
    var road = COLOR(neroChiaro)(CUBOID([cols*4,0.1,1]));
    road = T([Z])([zdistance])(road);
    zdistance = zdistance + 4;
    if(roads===null) roads = road;
    else roads = STRUCT([roads,road]);
  }
...
var roads1 = createRoads(7,5);

var settlement1 = createSettlement(7,5);
settlement1 = STRUCT([settlement1, roads1]);
...
var roads2 = createRoads(7,5);

var settlement2 = createSettlement(7,5);
settlement2 = STRUCT([settlement2, roads2]);
